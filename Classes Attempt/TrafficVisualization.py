...
class TrafficVisualization:
    ...
    def __init__(self, car_objects, road_length, road_radius, car_size, update_interval):
        ...
        self.center = (road_radius + 50, road_radius + 50)
        
        self.canvas = tk.Canvas(self.root, width=2*road_radius + 100, height=2*road_radius + 100)
        self.canvas.pack()
        self.draw_road()
    ...
    def update(self):
        ...
        for i, car in enumerate(self.cars):
            pos = self.car_objects[i].pos[self.current_step] / self.road_length
            if pos == 0:
                pos = 1
            angle = (2 * math.pi) * pos
            x = self.center[0] + self.road_radius * math.cos(angle)
